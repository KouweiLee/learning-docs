...
初始化：virtio_console_init

receive queue的notify函数：virtio_console_notify_rx

transmit queue的notify函数：virtio_console_notify_tx

当master fd可读时， 调用virtio_console_backend_read
...
acrn实际上是最开始由svm把控所有内存，然后把一部分内存分配给uvm。
