...
- [ ] 执行preadv和pwritev之前，可以合并多个breq为一个breq。要保证之后的请求肯定是在之后执行。这样可以使得顺序读写的性能会好一些。
...
- [x] 先实现Blk的，测试通了，再实现net的。
  ...
  - [x] 遇到了奇怪的问题，start zone之后，res_front还是0，但是别的cpu启动后，看到的res_front就是-1。不是里面的内容被改了，而是el2看到的改变了。。。。
...
- [x] 每个malloc，都记得要有一个free

- [x] 记得修改有参考acrn的代码
- [x] 考虑看看是否直接通过mmap将镜像加载进去

### 休眠：降低功耗

- [x] pause函数可以休眠直到信号出现：[c语言pause()函数（让进程暂停直到信号出现）-CSDN博客](https://blog.csdn.net/Dontla/article/details/122640811)；让进程暂停，会不会影响子线程？？？？我只想让主线程暂停。
- [x] **参考这里，让主线程来处理信号，从线程不动**：https://chat.openai.com/share/19f29791-d919-492e-9d80-dca6c51ca0c5
- [x] 不用担心信号遗漏的问题，信号只要在pending queue里，就ok。要担心的是没发信号。
...
- [x] 如果cpu此时因为配置请求阻塞了,之后先返回的是数据请求的结果,怎么办? 需要在返回的时候特判一下吗????
...
- [x] 另外rx_callback执行时, 每次都会报no more packets的错误, 解决它

这个不是错, 而是没有更多报文了.

- [ ] 目前有两个线程可以访问net, 需要考虑加锁. 不过先把没加锁的版本都跑通了, 之后再弄加锁版本的. 
- [ ] 许多地方需要加锁, 只要是多线程情况下就需要
